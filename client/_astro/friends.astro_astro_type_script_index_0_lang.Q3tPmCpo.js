/* empty css                        */class o{config;root;start=0;allArticles=[];container;randomArticleContainer;statsContainer;loadMoreBtn;modal;load(){this.loadMoreArticles(),this.loadMoreBtn.addEventListener("click",this.loadMoreArticles.bind(this)),window.onclick=t=>{const e=document.getElementById("modal");t.target===e&&this.hideModal()}}init(t){this.config={private_api_url:t.private_api_url||"",page_turning_number:t.page_turning_number||20,error_img:t.error_img||"https://fastly.jsdelivr.net/gh/willow-god/Friend-Circle-Lite@latest/static/favicon.ico"},this.root=document.getElementById("friend-circle-lite-root"),this.root&&(this.root.innerHTML="",this.createContainers())}createContainers(){this.randomArticleContainer=this.createElement("div",{id:"random-article"}),this.container=this.createElement("div",{className:"articles-container",id:"articles-container"}),this.loadMoreBtn=this.createElement("button",{id:"load-more-btn",innerText:"加载更多"}),this.statsContainer=this.createElement("div",{id:"stats-container"}),this.root.append(this.randomArticleContainer,this.container,this.loadMoreBtn,this.statsContainer)}createElement(t,e){const a=document.createElement(t);return Object.assign(a,e),a}loadMoreArticles(){const t="friend-circle-lite-cache",e="friend-circle-lite-cache-time",a=localStorage.getItem(e),r=Date.now();if(a&&r-Number(a)<600*1e3){const l=localStorage.getItem(t),i=l?JSON.parse(l):null;if(i){this.processArticles(i);return}}fetch(`${this.config.private_api_url}all.json`).then(l=>l.json()).then(l=>{localStorage.setItem(t,JSON.stringify(l)),localStorage.setItem(e,r.toString()),this.processArticles(l)}).finally(()=>{this.loadMoreBtn.innerText="Load more"})}processArticles({article_data:t,statistical_data:e}){this.allArticles=t,this.updateStats(e),this.displayRandomArticle(),this.displayArticles()}updateStats(t){this.statsContainer.innerHTML=`
    ...
    `,this.randomArticleContainer.querySelector("button#random-refresh")?.addEventListener("click",e=>{e.preventDefault(),this.displayRandomArticle()})}showAuthorArticles(t,e,a){if(!document.getElementById("fclite-modal")){const i=this.createElement("div",{id:"modal",className:"modal"});i.innerHTML=`
      ...
      `,this.root.appendChild(i)}this.modal=document.getElementById("modal");const r=document.getElementById("modal-articles-container");this.allArticles.filter(i=>i.author===t).slice(0,4).forEach(i=>{const s=`
        ...
        </div>`;r.insertAdjacentHTML("beforeend",s)}),this.modal.style.display="block",setTimeout(()=>{this.modal.classList.add("modal-open")},10)}hideModal(){this.modal.classList.remove("modal-open"),this.modal.addEventListener("transitionend",()=>{this.modal.style.display="none",this.root.removeChild(this.modal)},{once:!0})}}const n=new o;n.init({private_api_url:"https://fc.kemeow.top/",page_turning_number:10,error_img:"https://cravatar.cn/avatar/57d8260dfb55501c37dde588e7c3852c"});n.load();

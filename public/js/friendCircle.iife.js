var FriendCircle = (() => {
  ...
  // src/plugins/friendCircle.ts
  var friendCircle_exports = {};
  __export(friendCircle_exports, {
    FriendCircle: () => FriendCircle
  });
  var FriendCircle = class {
    static {
      __name(this, "FriendCircle");
    }
    config;
    root;
    start = 0;
    ...
    init(config) {
      this.config = {
        ...
        error_img: config.error_img || "https://fastly.jsdelivr.net/gh/willow-god/Friend-Circle-Lite@latest/static/favicon.ico"
      };
    ...
    loadMoreArticles() {
      ...
      const now = Date.now();
      if (cacheTime && now - Number(cacheTime) < 10 * 60 * 1e3) {
      ...
      fetch(`${this.config.private_api_url}all.json`).then((response) => response.json()).then((data) => {
    ...
    `;
      this.randomArticleContainer.querySelector("button#random-refresh")?.addEventListener("click", (event) => {
  ...
  return __toCommonJS(friendCircle_exports);
})();
